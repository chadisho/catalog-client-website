...
const RICH_TEXT_COLLAPSE_LENGTH = 260;
...
function parseDescription(rawDescription: string): ParsedDescription {
  ...
  if (quillDelta) {
    ...
    const html = sanitizeHtml(converter.convert());
  ...
  if (isLikelyHtml(rawDescription)) {
    const html = sanitizeHtml(rawDescription);
...
export default function ShopDescription({
  ...
  if (parsed.mode === 'text') {
    ...
    return (
      <>
        <p className="whitespace-pre-line text-sm leading-7 text-text/85">{content}</p>
        {hasLongText ? (
        ...
        <div
          className={
            hasLongRichText && !expanded
              ? 'max-h-40 overflow-hidden text-sm leading-7 text-text/85 [&_a]:text-primary [&_a]:underline [&_ol]:list-decimal [&_ol]:ps-5 [&_p]:mb-2 [&_ul]:list-disc [&_ul]:ps-5'
              : 'text-sm leading-7 text-text/85 [&_a]:text-primary [&_a]:underline [&_ol]:list-decimal [&_ol]:ps-5 [&_p]:mb-2 [&_ul]:list-disc [&_ul]:ps-5'
          }
