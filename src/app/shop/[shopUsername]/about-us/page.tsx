...
interface ShopAboutUsRouteParams {
  shopUsername: string;
}

interface ShopAboutUsPageProps {
  params: Promise<ShopAboutUsRouteParams>;
}
...
export async function generateMetadata({ params }: ShopAboutUsPageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      description,
      alternates: {
        canonical: `${SITE_URL}/shop/${shopUsername}/about-us`,
      },
      openGraph: {
        title,
        description,
        url: `${SITE_URL}/shop/${shopUsername}/about-us`,
        type: 'website',
        siteName: 'Chadisho',
        images: [
          {
...
export default async function Page({ params }: ShopAboutUsPageProps) {
  const { shopUsername } = await params;
