...
export async function generateMetadata({ searchParams }: CatalogViewPageProps): Promise<Metadata> {
  const resolvedSearchParams = await searchParams;
  const catalogCode = getSearchParam(resolvedSearchParams, 'catalogCode');
...
export default async function Page({ searchParams }: CatalogViewPageProps) {
  const resolvedSearchParams = await searchParams;
  ...
  if (!normalizedCatalogCode) {
    console.error('Catalog code is missing in route search params:', resolvedSearchParams);
